...
void get_col(int c, const std::vector<int>& in, std::vector<int>& out) 
{
	int collen = std::sqrt(in.size());
	out.clear();
	
	for (int j = 0; j < collen; j++)
	{
		out.push_back(in[twod_to_oned(j, c, collen)]);
	}
}
	// function get_subsq
...
{
	int rowlen = std::sqrt(in.size());
	int subsq_rowlen = std::sqrt(rowlen);
	out.clear();
	
	int i_offset = 0;
	int j_offset = subs * subsq_rowlen;
	while (j_offset > rowlen - 1)
	{
		j_offset -= rowlen;
		i_offset += subsq_rowlen;
	}
	
	for (int i = i_offset; i < i_offset + subsq_rowlen; i++)
	{
		for (int j = j_offset; j < j_offset + subsq_rowlen; j++)
		{
			out.push_back(in[twod_to_oned(i, j, rowlen)]);
		}
	}
}
